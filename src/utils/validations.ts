...
const is = {
    ...
    required: () => (value: Value): ErrorMessage =>
        isNilOrEmptyString(value) && "This file is required",

    minLength: (min: number) => (value: Value): ErrorMessage =>
        !!value && value.length < min && `Must be at least ${min} character`,

    maxLength: (max: number) => (value: Value): ErrorMessage =>
        !!value && value.length > max && `Must be one of: ${max} characters`,
